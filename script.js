...
function openModal(productId) {
  ...
  // 🔥 NUEVO: cargar likes desde Firebase
...
document.querySelectorAll(".reaction-btn").forEach(btn => {
  btn.addEventListener("click", function() {
    ...
    if (this.classList.contains("reacted")) {
      ...
      const reactionRef = ref(db, `reactions/${id}/${type}`);
      set(reactionRef, state.reactions[id][type]);
    }
...
function updateCartUI() {
  ...
  total.textContent = `$${totalPrice.toFixed(2)}`;
   ...
   // 🔥 AQUÍ VA LO NUEVO (GUARDAR EN FIREBASE)
  if (currentUser) {
    const userCartRef = ref(db, "carts/" + currentUser.uid);
    set(userCartRef, state.cart);
  }
}
...
document.getElementById("btnSave").addEventListener("click", () => {
  if (state.currentProduct) {
    saveProduct(state.currentProduct);
  }
});
...
function loadProfileCart(uid) {
  ...
  onValue(cartRef, snapshot => {
    ...
    if (!data.length) {
      container.innerHTML = "No tienes productos.";
      return;
    }
